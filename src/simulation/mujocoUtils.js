...
import { LacPolicyRunner } from './lac/policyRunner.js';
import {
  JOINT_NAMES_23DOF, JOINT_NAMES_ART, DEFAULT_JOINT_POS_23, STIFFNESS_23, DAMPING_23,
} from './lac/constants.js';
...
export async function reloadScene(mjcf_path) {
  ...
  // rebind the native-perturb interaction to the fresh model/data (recreates MjvScene)
  if (this.dragStateManager) {
    this.dragStateManager.setMujoco(mujoco, this.model, this.data);
  }
}
...
export async function reloadPolicy(policy_path, options = {}) {
  ...
  if (config.lac) {
    await reloadLacPolicy.call(this, config);
    return;
  }
...
// ---- LAC deploy path ----
// Build the LacPolicyRunner, the ART-order joint maps for obs, and the standing
// init pose. The viewer's external-PD control loop is driven by the J23 mapping
// (configureJointMappings) + kp/kd, identical to the stock path.
export function setLacInitialPose(demo) {
  demo.simulation.resetData();
  const qpos = demo.simulation.qpos;
  const qvel = demo.simulation.qvel;
  qpos[0] = 0.0; qpos[1] = 0.0; qpos[2] = 0.793;     // base pos
  qpos[3] = 1.0; qpos[4] = 0.0; qpos[5] = 0.0; qpos[6] = 0.0;  // base quat (w,x,y,z)
  for (let i = 0; i < demo.numActions; i++) {
    // Legs (J23 idx 0-11) get up-to-±5° init noise: an exactly-zero symmetric leg state
    // triggers a slow postural lean when standing long (user observation). Init-only —
    // the gait never revisits the exact-zero state afterwards.
    const noise = i < 12 ? (Math.random() * 2 - 1) * (5 * Math.PI / 180) : 0;
    qpos[demo.qpos_adr_policy[i]] = DEFAULT_JOINT_POS_23[i] + noise;   // J23 order
  }
  for (let i = 0; i < qvel.length; i++) qvel[i] = 0.0;
  demo.simulation.forward();
}

async function reloadLacPolicy(config) {
  // J23 mapping -> ctrl/qpos/qvel adr + numActions (drives the external-PD loop).
  configureJointMappings(this, JOINT_NAMES_23DOF);
  // ART mapping -> qpos/qvel adr for joint_pos_rel / joint_vel_rel obs terms.
  this.qpos_adr_art = [];
  this.qvel_adr_art = [];
  for (const name of JOINT_NAMES_ART) {
    const jointIdx = this.jointNamesMJC.indexOf(name);
    if (jointIdx < 0) throw new Error(`LAC ART joint "${name}" not found in model`);
    this.qpos_adr_art.push(this.model.jnt_qposadr[jointIdx]);
    this.qvel_adr_art.push(this.model.jnt_dofadr[jointIdx]);
  }
  this.defaultJposPolicy = Float32Array.from(DEFAULT_JOINT_POS_23);
  this.kpPolicy = Float32Array.from(STIFFNESS_23);
  this.kdPolicy = Float32Array.from(DAMPING_23);
  this.control_type = 'joint_position';

  setLacInitialPose(this);

  this.policyRunner = new LacPolicyRunner(config);
  await this.policyRunner.init();
  this.policyRunner.reset();

  this.lacPresets = null;
  if (config.presets_path) {
    try {
      const r = await fetch(config.presets_path);
      if (r.ok) this.lacPresets = await r.json();
    } catch (e) { console.warn('LAC presets load failed', e); }
  }
}
...
export async function loadSceneFromURL(mujoco, filename, parent) {
  ...
  for (let g = 0; g < model.ngeom; g++) {
    // Render visual geoms (group 1-2) + ground planes; skip group-0 collision geoms
    // (the 23-dof MJCF puts visual meshes at group 1 and collision shapes at group 0).
    const _isPlane = (model.geom_type[g] == mujoco.mjtGeom.mjGEOM_PLANE.value);
    if (!_isPlane && !(model.geom_group[g] >= 1 && model.geom_group[g] < 3)) { continue; }
